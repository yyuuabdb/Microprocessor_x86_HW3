...
; 데이터 및 스택 세그먼트 초기화
    ; ========================================================
    mov ax, SYS_DATA_SEL    ; 데이터 세그먼트 셀렉터 (0x10) 로드
    mov ds, ax              ; DS 레지스터 초기화
    mov ss, ax              ; SS 레지스터 초기화
    mov esp, 0x9000         ; 32비트 스택 포인터 명시적 설정
    ; ========================================================
...
; control transfer
	jmp SYS_CODE_SEL_1:GDT_Return	;;GDT_Return으로 복귀
...
;ldt1_2:
	...
	db 0cfh
	db 0
ldt1_end:					;;Limit 계산을 위한 라벨
	
ldt2:
LDT2_DATA_SEL_0 equ 04h		;;LDT2 Index 0 (Data)
	dw 0FFFFh
	dw 00000h
	db 0
	db 92h
...
LDT2_Call_GATE equ 14h
	...
	db 0					; Param Count = 0
	db 8Ch					; Type = Call Gate (8C)
	dw 0					; Offset 31:16
	
LDT2_CODE_SEL_1 equ 1Ch		;;LDT2 Index 3 (Code)
	dw 0FFFFh
	dw 00000h
	db 0
	db 9Ah
	db 0cfh
	db 0
ldt2_end:					;;Limit 계산을 위한 라벨

ldt3:
LDT3_CODE_SEL_0 equ 04h		;;LDT3 Index 0 (Code)
	dw 0FFFFh
	dw 00000h
	db 0
	db 9Ah
...
LDT3_DATA_SEL_0 equ 0Ch		;;LDT3 Index 1 (Data)
	dw 0FFFFh
	dw 00000h
	db 0
	db 92h
